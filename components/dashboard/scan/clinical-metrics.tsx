  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Metric
        ...
        unit="Hz"
        sub="Higher = stiffer, more consolidated bone"
      />
      <Metric
        ...
        sub={
          m.zeta < 0.04
            ? "Solid · healed energy retention"
            : m.zeta < 0.10
            ? "Stable · still gaining stiffness"
      ...
      <Metric
        ...
        unit="Hz"
        sub="Width of resonance at −3 dB"
      />
      <Metric
        label="Modal damping factor"
        value={m.mdf.toFixed(4)}
        sub="Log-decrement ÷ 2π · energy loss per cycle"
      />
    </div>
  );
}
...
function Metric({
  ...
  return (
    <div className="surface p-5">
      ...
      <div className="mt-1 flex items-baseline gap-1 font-mono leading-none">
...
function RustMetric({ m }: { m: ClinicalMetrics }) {
  ...
  const labelOf = (n: number) =>
    n >= 9 ? "Bridging callus" : n >= 6 ? "Early callus" : "No callus";
  ...
  return (
    <div className="surface p-5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-1.5">
        {(["Anterior", "Posterior", "Medial", "Lateral"] as const).map((side) => {
          const key = side.toLowerCase() as keyof typeof m.rustCortex;
          const score = m.rustCortex[key];
          const color = cortexColor(score);
          return (
            <div
              key={side}
            ...
            >
              <span className="text-[11px] text-text-muted">{side}</span>
